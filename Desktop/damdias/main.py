...
async def choose_language(message: types.Message, state: FSMContext):
    if message.text == "Русский 🇷🇺":
        ...
        payment_text = "Пожалуйста, отправьте скриншот или PDF-файл оплаты (Kaspi/Halyk)"
        ...
        payment_text = "Төлемнің скриншотын немесе PDF файлын жіберіңіз (Kaspi/Halyk)"
    ...
    await state.update_data(language=lang, menu=menu,
...
@dp.message(StateFilter(OrderStates.waiting_for_payment),
            F.content_type.in_([ContentType.TEXT, ContentType.DOCUMENT, ContentType.PHOTO]))
async def payment(message: types.Message, state: FSMContext):
    data = await state.get_data()
    if message.document:
        if not message.document.file_name.lower().endswith(".pdf"):
            await message.answer("Пожалуйста, отправьте PDF-файл или ссылку на оплату.")
            return
        payment_info = f"PDF-файл: {message.document.file_name}"
        await bot.send_document(ADMIN_ID, message.document.file_id, caption=f"Заказ №{data['order_id']}")
    elif message.photo:
        payment_info = "Скриншот оплаты"
        await bot.send_photo(ADMIN_ID, message.photo[-1].file_id, caption=f"Заказ №{data['order_id']}")
    else:
...
if __name__ == "__main__":
    print("✅ Бот запущен на Render.com и работает 24/7")
